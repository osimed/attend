...
class CalendarPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocListener<DatabaseSyncBloc, DatabaseSyncState>(
      listener: (context, state) {
        if (state is DatabaseSyncSyncedMode && state.isSuccess) {
          context.read<CalendarGridBloc>().add(LoadMonthlyCalendar());
        }
      },
      child: BlocListener<HeaderPanelBloc, HeaderPanelState>(
        listener: (context, state) {
              ...
              IconButton(
                ...
                onPressed: () {
                  Navigator.of(context).push(
                    MaterialPageRoute(
                      builder: (_) => BlocProvider.value(
                        value: locator.get<DatabaseSyncBloc>(),
